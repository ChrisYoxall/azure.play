...
variable "resource_prefix_name" {
  description = "The name to use to prefix created resources in Azure"
  type        = string
  default     = "demo"
  validation {
    condition     = length(var.resource_prefix_name) > 0
    error_message = "Must provide a resource prefix name"
  }
}
